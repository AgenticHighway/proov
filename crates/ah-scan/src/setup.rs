...
pub fn run_setup(force: bool) -> bool {
    ...
    if key.is_empty() {
        ...
        eprintln!("  {DIM}Run `ah-scan setup` later to configure an API key.{RESET}");
        eprintln!();
        return false;
    }

    let endpoints = &[
        "Agentic Highway Cloud (verify.agentichighway.ai)",
...
fn print_welcome() {
    eprintln!();
    eprintln!("  {DIM}┌──────────────────────────────────────────┐{RESET}");
    eprintln!("  {DIM}│{RESET}  {BOLD}{CYAN}ah-scan{RESET}  —  First-Time Setup             {DIM}│{RESET}");
    eprintln!("  {DIM}└──────────────────────────────────────────┘{RESET}");
    eprintln!();
}
